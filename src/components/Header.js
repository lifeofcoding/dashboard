...
class Header extends Component {
    ...
    searchJobs(search) {
        ...
        try {
            ...
            axios.get('/search/' + search).then((res) => {
                ...
                JobActions.add(res.data.results);
                this.setState({
                    showResults: true,
                    isLoading: false
                });

                this.props.location.push('search/' + this.state.input);
            });
        } catch (error) {
            this.setState({
                error,
    ...
    onSubmit(e) {
        ...
        this.searchJobs(this.state.input)
    }
    ...
    render() {
      /*
        if (this.state.showResults) {
          return (
            <Route path="*" render={() => (
              <Redirect to={'search/' + this.state.input}/>
            )}/>
          );
        }
      */
        ...
        return (
            <section>
                <header className="header-desktop">
                    <div className="section__conte:nt section__content--p30">
                        <div className="container-fluid">
                            <div className="header-wrap">
                                <form className="form-header" action="" method="POST" onSubmit={this.onSubmit}>
                                    <input className="au-input au-input--xl" type="text" name="search" value={this.state.input}
                                        placeholder="Search for datas &amp; reports..." onChange={this.handleInputChange}
                                    />
